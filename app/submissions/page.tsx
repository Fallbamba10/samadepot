...
const STEPS: { key: SubmissionStatus | "__"; label: string }[] = [
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.key} className="flex flex-1 flex-col items-center gap-1">
            ...
            {active && (
              <span className="text-[10px] font-bold text-brand-600">{step.label}</span>
            )}
          </div>
        );
      })}
      ...
      <div className="flex items-start justify-between gap-4 px-4 pb-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {submission.isLate && (
              <span className="shrink-0 rounded-full bg-coral-50 px-2 py-0.5 text-[10px] font-bold text-coral-500">
                Retard
              </span>
            )}
          </div>
          <p className="mt-0.5 truncate text-xs text-muted">
            {submission.fileName} · {submission.sizeMb} Mo
          </p>
...
export default async function SubmissionsPage() {
  ...
  const pending = submissions.filter((s) =>
    ["received", "read", "late"].includes(s.status)
  );
  const validated = submissions.filter((s) =>
    ["validated", "graded"].includes(s.status)
  );
    ...
    <AppShell active="Depots">
      ...
      {submissions.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-line bg-white p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted">
            Tes travaux déposés apparaîtront ici avec leur statut en temps réel.
          </p>
          <Link
            href="/spaces"
          ...
          {attention.length > 0 && (
            <section>
              <div className="mb-3 flex items-center gap-2">
                ...
                <span className="rounded-full bg-saffron-50 px-2 py-0.5 text-xs font-bold text-saffron-500">
                  {attention.length}
                </span>
              </div>
          ...
          {pending.length > 0 && (
            <section>
              <div className="mb-3 flex items-center gap-2">
                ...
                <span className="rounded-full bg-lagoon-50 px-2 py-0.5 text-xs font-bold text-lagoon-500">
                  {pending.length}
                </span>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
